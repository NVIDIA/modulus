...
def halo_unpadding_1d(
    ...
    if mesh_dim is None:
        assert mesh.ndim == 1, f"Halo padding requires `dim` for mesh size > 1 (got shape {mesh.shape})"
        mesh_dim = 0
...
def halo_padding_1d(
    ...
    assert edge_padding_t in valid_padding, f"Invalid edge padding: {edge_padding_t}"
...
def perform_halo_collective(
    ...
    if method == "p2p":
        ...
        halo_from_right = all_to_all_recv[local_rank + 1] if local_rank != local_size - 1 else None
