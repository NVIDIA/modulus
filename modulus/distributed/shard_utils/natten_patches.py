...
def shard_to_haloed_local(
    ...
    Raises:
        AssertionError: If q/k/v are not sharded on same mesh
    """
...
class HaloPaddingND(torch.autograd.Function):
    ...
    def forward(
        ...
        Raises:
            AssertionError: If halo size doesn't match mesh dimensions
        """
...
class UnSliceHaloND(torch.autograd.Function):
    ...
    def forward(
        ...
        Raises:
            AssertionError: If halo size does not match mesh rank
        """
        # Save context for backward pass
        ctx.halo = halo
        ctx.mesh = mesh 
...
if natten_spec is not None:
    ...
    def na2d_wrapper(wrapped: Any, instance: Any, args: tuple, kwargs: dict) -> Union[torch.Tensor, ShardTensor]:
        ...
        """
        def fetch_qkv(q: Any, k: Any, v: Any, *args: Any, **kwargs: Any) -> Tuple[Any, Any, Any]:
