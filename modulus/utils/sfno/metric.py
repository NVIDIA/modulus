...
# we need those here:
from modulus.utils.sfno.metrics.weighted_acc_rmse import Quadrature, lat_torch, latitude_weighting_factor_torch, l1_torch_local, l1_torch_distributed, weighted_rmse_torch_local, weighted_rmse_torch_distributed, weighted_acc_torch_local, weighted_acc_torch_distributed
...
class MetricsHandler():
    ...
    def __init__(self, params, mult, clim, device,
        ...
        lat_full = lat_torch(torch.arange(start=0, end=params.img_crop_shape_x, device=self.device), params.img_crop_shape_x)
        ...
        lwf = latitude_weighting_factor_torch(lat_full, params.img_shape_x, lat_norm)[params.img_crop_offset_x:params.img_crop_offset_x+params.img_crop_shape_x]
        ...
        if params.multigrid_mode == "distributed":
            self.metric_correction_factor = 1. / float(comm.get_size("matmul"))
            ...
            self.metric_correction_factor = 1.
    ...
    def initialize_buffers(self):
        ...
        self.valid_weighted_rmse = torch.zeros( (self.out_channels_local), dtype=torch.float32, device=self.device)
    ...
    def update(self, prediction, target, loss, idt):
        ...
        l1 = self.l1_handle(prediction, target) * self.metric_correction_factor
        ...
        # update parameters
        self.acc_curve[:, idt] += self.weighted_acc_handle(prediction - self.clim, target - self.clim, self.latitude_weights) * self.metric_correction_factor
        self.acc_counter[idt] += 1
        if idt == 0:
            self.valid_steps += 1.
            self.valid_loss += loss
            self.valid_l1 += l1
            self.valid_weighted_rmse += self.weighted_rmse_handle(rmse_prediction, rmse_target, self.latitude_weights) * self.metric_correction_factor
    ...
    def finalize(self, final_inference=False):
        ...
        with torch.no_grad():
            ...
            if dist.is_initialized():
                ...
                dist.all_reduce(self.valid_weighted_rmse, op=dist.ReduceOp.SUM, group = comm.get_group("data"))
            ...
            if self.split_data_channels:
                # gather rmse
                valid_weighted_rmse_list = torch.split(torch.zeros( (self.N_out_channels), dtype=torch.float32, device=self.device), self.out_channels_local, dim=0)
                valid_weighted_rmse_list = [x.contiguous() for x in valid_weighted_rmse_list]
                valid_weighted_rmse_list[comm.get_rank("matmul")] = self.valid_weighted_rmse
                dist.all_gather(valid_weighted_rmse_list, self.valid_weighted_rmse, group=comm.get_group("matmul"))
            ...
            self.valid_weighted_rmse = self.mult * self.valid_weighted_rmse / self.valid_buffer[2]

            # Pull out autoregessive acc values
            self.acc_curve /= self.acc_counter

            # prepare logs with the minimum content
            valid_buffer_cpu = self.valid_buffer.detach().cpu().numpy()
            logs = {'base': {'validation steps' : valid_steps_local,
                             'validation loss': valid_buffer_cpu[0],
                             'validation L1': valid_buffer_cpu[1]},
                    "metrics": {}}

            for var_name, var_idx in self.rmse_vars.items():
                logs["metrics"]['validation ' + var_name] = self.valid_weighted_rmse[var_idx].detach().cpu().numpy()

            for var_name, var_idx in self.acc_vars.items():
                logs["metrics"]['ACC time ' + var_name] = self.acc_curve[var_idx, self.valid_autoreg_steps].detach().cpu().numpy()

            for var_name, var_idx in self.acc_auc_vars.items():
                logs["metrics"]['ACC AUC ' + var_name] = self.simpquad(self.acc_curve[var_idx, :], dim=0).detach().cpu().numpy()
