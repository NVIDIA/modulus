...
class MultigridPatches2D(object):
    def __init__(self, levels=1, padding=0, multi_pass=False, sampling_mode="dilation"): # pragma: no cover
        ...
        self.num_col_gpu = 1
        assert (self.grid_len * self.grid_len) % comm.get_size(
            "matmul"
        ) == 0, "Error, make sure that the number of gpus can divide the full patchgrid"

        while not (
            (self.grid_len % self.num_row_gpu == 0)
            and (self.grid_len % self.num_col_gpu == 0)
        ):
            self.num_row_gpu /= 2
            self.num_col_gpu *= 2
            ...
            if self.num_col_gpu > comm.get_size("matmul"):
                raise ValueError(
                    f"Error, no suitable decomposition found for {self.grid_len} x {self.grid_len} grid and {comm.get_size('matmul')} gpus"
                )
    ...
    def _make_patches(
        ...
        if safe_pad:
            size = x.size()
            pad_first = [
                min([padding[1], size[3]]),
                min([padding[1], size[3]]),
                min([padding[0], size[2]]),
                min([padding[0], size[2]]),
            ]
            pad_second = [
                max([padding[1] - size[3], 0]),
                max([padding[1] - size[3], 0]),
                max([padding[0] - size[2], 0]),
                max([padding[0] - size[2], 0]),
            ]
            ...
            x_pad = torch.nn.functional.pad(
                x, pad=[padding[1], padding[1], padding[0], padding[0]], mode="circular"
            )
    ...
    def _patch_level(
        ...
        if sampling_mode == "dilation":
            ...
            x_sub = torch.nn.functional.avg_pool2d(
                x, kernel_size=sub_sample, stride=sub_sample
            )
        ...
        padding = [
            math.ceil(
                ((output_size[0] - 1) * stride[0] - size[2] + patch_size[0]) / 2.0
            )
            + self.padding[0],
            math.ceil(
                ((output_size[1] - 1) * stride[1] - size[3] + patch_size[1]) / 2.0
            )
            + self.padding[1],
        ]
        ...
        if self.multi_pass:
            ...
            stride = [
                patch_size[0] // self.num_row_gpu,
                patch_size[1] // self.num_col_gpu,
            ]
            ...
            x_local = self._make_patches(
                x_pad, kernel_size=kernel_size, stride=stride, mode="separate"
            )
            x_local = torch.squeeze(
                scatter_to_matmul_parallel_region(x_local, dim=1), dim=1
            )

            # now do the second step
            stride = [
                patch_size[0] // self.num_row_patch,
                patch_size[1] // self.num_col_patch,
            ]
            kernel_size = [stride[0] + 2 * padding[0], stride[1] + 2 * padding[1]]
            ...
            x_level = self._make_patches(
                x_local, kernel_size=kernel_size, stride=stride, mode="batch-wise"
            )
        else:
            stride = [patch_size[0] // sub_sample, patch_size[1] // sub_sample]
            kernel_size = [
                patch_size[0] + 2 * self.padding[0],
                patch_size[1] + 2 * self.padding[1],
            ]
    ...
    def patch(self, x: torch.Tensor) -> torch.Tensor: # pragma: no cover
        ...
        if self.multi_pass:
            ...
            stride = [
                (patched_local.size(2) - 2 * self.padding[0]) // self.num_row_patch,
                (patched_local.size(3) - 2 * self.padding[1]) // self.num_col_patch,
            ]
            # compute kernel_size:
            kernel_size = [
                stride[0] + 2 * self.padding[0],
                stride[1] + 2 * self.padding[1],
            ]
            # do the final patching
            patched = self._make_patches(
                patched_local,
                kernel_size=kernel_size,
                stride=stride,
                padding=0,
        ...
        patch_size = [
            patched.size(2) - 2 * self.padding[0],
            patched.size(3) - 2 * self.padding[1],
        ]
    ...
    def stitch_global(
        ...
        x_stitch = stitch_patches(
            x_global,
            n=[self.num_row_gpu, self.num_col_gpu],
            p=padding,
            mode="batch-wise",
        )
    ...
    def stitch(
        ...
        if self.multi_pass:
            ...
            x_global = x_global.reshape(-1, *x_global.shape[2:])
            x = stitch_patches(
                x_global, n=[self.grid_len, self.grid_len], p=padding, mode="batch-wise"
            )
