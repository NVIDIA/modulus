...
class LossHandler(nn.Module):
    ...
    def __init__(self, params, img_size=(720,1440), d=2):
        ...
        if loss_type[:8] == 'weighted':
            ...
            channel_weights = params.N_out_channels * channel_weights / torch.sum(channel_weights)
        ...
        if loss_type == 'l2':
            self.loss_obj = DistributedLpLoss(d=d, p=2) if params.split_data_channels else LpLoss(d=d, p=2)
        elif loss_type == 'l1':
            self.loss_obj = DistributedLpLoss(d=d, p=1) if params.split_data_channels else LpLoss(d=d, p=1)
        elif loss_type == 'geometric l2':
            self.loss_obj = GeometricLpLoss(self.img_shape, p=2, absolute=absolute, squared=squared, pole_mask=pole_mask)
        elif loss_type == 'geometric l1':
            self.loss_obj = GeometricLpLoss(self.img_shape, p=1, absolute=absolute, pole_mask=pole_mask)
            ...
            self.loss_obj = SpectralLoss(self.img_shape, absolute=absolute, squared=squared)
        else:
    ...
    def forward(self, prd: torch.Tensor, tar: torch.Tensor, inp: torch.Tensor):
        ...
        if self.training:
            ...
            prd = prd.reshape(B, -1, C // (self.n_future+1), H, W) * self.multistep_weight
            tar = tar.reshape(B, -1, C // (self.n_future+1), H, W) * self.multistep_weight
            ...
            if hasattr(self, 'dynamic_weight_obj'):
                ...
                dynamic_weight = dynamic_weight / torch.sum(dynamic_weight, dim=-1, keepdim=True)
...
class GeometricLpLoss(nn.Module):
    def __init__(self, img_size: Tuple[int, int], p: Optional[float]=2., size_average: Optional[bool]=True,
    ...
    def abs(self, prd: torch.Tensor, tar: torch.Tensor, chw: torch.Tensor):
        ...
        if self.pole_mask:
            all_norms = torch.sum(torch.abs(prd[..., self.pole_mask:-self.pole_mask, :]-tar[..., self.pole_mask:-self.pole_mask, :])**self.p * self.jacobian[..., self.pole_mask:-self.pole_mask, :] * self.dA, dim=(-2,-1))
        else:
            all_norms = torch.sum(torch.abs(prd-tar)**self.p * self.jacobian * self.dA, dim=(-2,-1))
    ...
    def rel(self, prd: torch.Tensor, tar: torch.Tensor, chw: torch.Tensor, mask: Optional[torch.Tensor] = None):
        num_examples = prd.size()[0]

        if self.pole_mask:
            diff_norms = torch.sum(torch.abs(prd[..., self.pole_mask:-self.pole_mask, :]-tar[..., self.pole_mask:-self.pole_mask, :])**self.p * self.jacobian[..., self.pole_mask:-self.pole_mask, :] * self.dA, dim=(-2,-1))
        else:
            diff_norms = torch.sum(torch.abs(prd-tar)**self.p * self.jacobian * self.dA, dim=(-2,-1))
        
        diff_norms = diff_norms.reshape(num_examples, -1)

        tar_norms = torch.sum(torch.abs(tar)**self.p * self.jacobian * self.dA, dim=(-2,-1))
    ...
    def forward(self, prd: torch.Tensor, tar: torch.Tensor, chw: torch.Tensor, mask: Optional[torch.Tensor] = None):
        if self.absolute:
...
class LpLoss(nn.Module):
    ...
    def abs(self, prd: torch.Tensor, tar: torch.Tensor, chw: torch.Tensor):
        ...
        all_norms = (h**(self.d/self.p)) * torch.linalg.norm(prdv-tarv, ord=self.p, dim=1)
...
class SpectralLoss(nn.Module):
    ...
    def rel(self, prd: torch.Tensor, tar: torch.Tensor, mask: Optional[torch.Tensor] = None):
        ...
        tar_coeffs = tar_coeffs[..., 0]**2 + tar_coeffs[..., 1]**2
        tar_norm2 = tar_coeffs[..., :, 0] + 2 * torch.sum(tar_coeffs[..., :, 1:], dim=-1)
        tar_norm2 = (self.spectral_weights * tar_norm2).reshape(num_examples, -1).sum(dim=-1)
