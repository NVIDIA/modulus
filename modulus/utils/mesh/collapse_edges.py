...
def load_collapse_edges_save(file_path: str, output_path: str, rel_threshold: float = 10.0, preserve_feature: bool = True) -> None:
    """
    ...
    """
    supported_formats = [".obj", ".ply", ".off", ".stl", ".mesh", ".node", ".poly", ".msh"]
    _, input_extension = os.path.splitext(file_path)
    if input_extension not in supported_formats:
        raise ValueError("Unsupported file format. Supported formats are: " + ", ".join(supported_formats))
...
def collapse_edges(vertices: np.ndarray, faces: np.ndarray, rel_threshold: float = 10.0, preserve_feature: bool = True) -> Tuple[np.ndarray, np.ndarray, dict]:
    """
    ...
    vertices, faces, information = pymesh.collapse_short_edges_raw(mesh.vertices, mesh.faces, rel_threshold=rel_threshold, preserve_feature=preserve_feature)
