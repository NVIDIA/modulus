...
def split_trajectory(context, features, window_length=7, predict_length=10):
  ...
  model_input_features['senders'] = tf.tile(
      tf.expand_dims(context['senders'], axis=0),
      [input_trajectory_length, 1])

  model_input_features['receivers'] = tf.tile(
      tf.expand_dims(context['receivers'], axis=0),
      [input_trajectory_length, 1])
  ...
  if 'step_context' in features:
    ...
    for idx in range(input_trajectory_length):
        ...
        zero_pad = tf.zeros([trajectory_length-read_step_context.shape[0]-1, 1], dtype=features['step_context'].dtype)
  ...
  pos_stack = []
  for idx in range(input_trajectory_length):
    pos_stack.append(features['position'][idx:idx + window_length + predict_length])
