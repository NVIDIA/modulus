...
from mpu.fft3d import RealFFT3, InverseRealFFT3, DistributedRealFFT3, DistributedInverseRealFFT3
...
def main(args, verify):
    ...
    backward_transform_local = InverseRealFFT3(nd=D, nh=H, nw=W).to(device)
    backward_transform_dist = DistributedInverseRealFFT3(nd=D, nh=H, nw=W).to(device)
    Lpad = backward_transform_dist.lhpad
    Mpad = backward_transform_dist.lwpad
    Lloc = (Lpad + backward_transform_dist.lhmax) // h_parallel_size
    Mloc = (Mpad + backward_transform_dist.lwmax) // w_parallel_size


    # set up inputs
    dummy_full = torch.randn((B, C, D, H, W), dtype=torch.float32, device=device)
    ...
    with torch.no_grad():
        ...
        if w_parallel_size > 1:
            ...
            igrad_full_gather = igrad_full_gather[..., :backward_transform_dist.lwmax].contiguous()
        else:
        ...
        if h_parallel_size > 1:
            ...
            igrad_full_gather = igrad_full_gather[..., :backward_transform_dist.lhmax, :].contiguous()
