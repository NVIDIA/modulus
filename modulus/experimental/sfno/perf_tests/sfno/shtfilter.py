...
def main(args, verify):
    ...
    # blocks
    model_loc = nn.Sequential(
        *[
            RealSphericalHarmonicTransform(H, W, grid="equiangular"),
            InverseRealSphericalHarmonicTransform(H, W, grid="equiangular"),
        ]
    ).to(device)
    ...
    max_mem_gb = torch.cuda.max_memory_allocated(device=device) / (
    ...
    with torch.autograd.profiler.emit_nvtx(
        enabled=args.enable_profiling, record_shapes=False
    ):
        ...
        if dist.is_initialized():
            dist.barrier(
                device_ids=[device.index], group=comm.get_model_parallel_group()
            )
