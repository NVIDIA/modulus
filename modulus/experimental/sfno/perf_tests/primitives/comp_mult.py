...
class ComplexMult(nn.Module):
    def __init__(
        self, num_blocks, block_size, hidden_size_factor, use_complex_kernels=True
    ):
        super(ComplexMult, self).__init__()
        self.weight = nn.Parameter(
            torch.ones(
                (num_blocks, block_size, block_size * hidden_size_factor, 2),
                dtype=torch.float32,
            )
        )
        self.bias = nn.Parameter(
            torch.zeros(
                (num_blocks, block_size * hidden_size_factor, 1, 1, 2),
                dtype=torch.float32,
            )
        )
        self.mult_handle = (
...
def main():
    ...
    os.environ["LOCAL_RANK"] = str(
        int(os.getenv("WORLD_RANK", "0")) % torch.cuda.device_count()
    )
    ...
    # we need those
    inp = torch.ones(
        (batch_size, hidden_size, H, W // 2 + 1, 2), dtype=torch.float32, device=device
    )
    model = ComplexMult(
        num_blocks,
        block_size,
