...
# helpers
from modulus.experimental.sfno.networks.layers import MLP, DropPath


class PatchEmbed(nn.Module):
    def __init__(
        self, img_size=(224, 224), patch_size=(16, 16), inp_chans=3, embed_dim=768
    ):
        ...
        self.proj = nn.Conv2d(
            inp_chans, embed_dim, kernel_size=patch_size, stride=patch_size
        )
...
class AFNO2D(nn.Module):
    def __init__(
        ...
        self.b1 = nn.Parameter(
            self.scale * torch.randn(1, self.num_blocks * self.block_size, 1, 1)
        )
    ...
    def forward(self, x):
        ...
        if kept_modes_H == total_modes_H:
            oac = torch.view_as_complex(
                self.mult_handle(x[:, :, :, :, :kept_modes_W, :], self.w1)
            )
            oa = torch.view_as_real(self.act(oac))
            x_fft[:, :, :, :, :kept_modes_W, :] = self.mult_handle(oa, self.w2)
        else:
            olc = torch.view_as_complex(
                self.mult_handle(x[:, :, :, :kept_modes_H, :kept_modes_W, :], self.w1)
            )
            ohc = torch.view_as_complex(
                self.mult_handle(x[:, :, :, -kept_modes_H:, :kept_modes_W, :], self.w1)
            )
            ...
            x_fft[:, :, :, :kept_modes_H, :kept_modes_W, :] = self.mult_handle(
                ol, self.w2
            )
            x_fft[:, :, :, -kept_modes_H:, :kept_modes_W, :] = self.mult_handle(
                oh, self.w2
...
class AdaptiveFourierNeuralOperatorNet(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        assert (
            len(patch_size) == 2
        ), f"Expected patch_size to have two entries but got {patch_size} instead"
    ...
    def _init_weights(self, m):
        if isinstance(m, nn.Linear) or isinstance(m, nn.Conv2d):
            ...
            or isinstance(m, FusedLayerNorm)
