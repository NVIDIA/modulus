...
from modulus.experimental.sfno.mpu.layer_norm import (
...
from modulus.experimental.sfno.networks.layers import (
    MLP,
    DropPath,
    EncoderDecoder,
    InverseRealFFT2,
...
from modulus.experimental.sfno.networks.spectral_convolution import (
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if spectral_transform == "sht":
            ...
            inp_dist_h = (self.inp_shape[0] + comm.get_size("h") - 1) // comm.get_size(
                "h"
            )
            inp_dist_w = (self.inp_shape[1] + comm.get_size("w") - 1) // comm.get_size(
                "w"
            )
            self.inp_padding = (
            ...
            out_dist_h = (self.out_shape[0] + comm.get_size("h") - 1) // comm.get_size(
                "h"
            )
            out_dist_w = (self.out_shape[1] + comm.get_size("w") - 1) // comm.get_size(
                "w"
            )
            self.out_padding = (
