...
from modulus.experimental.sfno.networks.contractions import (
    _contract_rank,
    compl_exp_mul2d_fwd,
...
class SpectralConv(nn.Module):
    ...
    def _init_weights(self):
        ...
        self.scale = math.sqrt(0.1 / float(in_channels + out_channels))
        nn.init.normal_(self.weight, mean=0.0, std=self.scale)
...
class SpectralAttention(nn.Module):
    ...
    def __init__(
        ...
        if operator_type == "diagonal":
            ...
            w = [
                scale
                * torch.randn(self.in_channels, hidden_size, dtype=torch.complex64)
            ]
            for l in range(1, self.spectral_layers):
                w.append(
                    self.scale
                    * torch.randn(hidden_size, hidden_size, dtype=torch.complex64)
                )
            self.w = nn.ParameterList(w)

            self.wout = nn.Parameter(
                scale
                * torch.randn(hidden_size, self.out_channels, dtype=torch.complex64)
            )

            if bias:
                self.b = nn.ParameterList(
                    [
                        scale * torch.randn(hidden_size, 1, 1, dtype=torch.complex64)
            ...
            w = [
                scale
                * torch.randn(
                    self.modes_lat, self.in_channels, hidden_size, dtype=torch.complex64
                )
            ]
            for l in range(1, self.spectral_layers):
                w.append(
                    scale
                    * torch.randn(
                        self.modes_lat, hidden_size, hidden_size, dtype=torch.complex64
                    )
                )
            self.w = nn.ParameterList(w)
