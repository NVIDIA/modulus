...
def _sync_module_states(
    ...
    with torch.no_grad():
        # sync real and complex values separately
        module_states: List[torch.Tensor] = []
        for name, param in module.named_parameters():
            if (name not in params_and_buffers_to_ignore) and (not param.is_complex()):
        ...
        for name, param in module.named_parameters():
            if (name not in params_and_buffers_to_ignore) and (param.is_complex()):
                param.copy_(torch.view_as_complex(module_states[count]))
                count += 1
