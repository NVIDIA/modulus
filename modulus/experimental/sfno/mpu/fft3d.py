...
class RealFFT3(nn.Module):
    ...
    def forward(self, x):
        y = torch.fft.rfftn(
            x, s=(self.nd, self.nh, self.nw), dim=(-3, -2, -1), norm="ortho"
        )
        ...
        # truncate in h
        yt = torch.cat(
            [yt[..., : self.lhmax_high, :], y[..., -self.lhmax_low :, :]], dim=-2
        )

        # truncate in d
        y = torch.cat(
            [yt[..., : self.ldmax_high, :, :], y[..., -self.ldmax_low :, :, :]], dim=-3
        )
...
class DistributedRealFFT3(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        xtftt = torch.cat(
            [xtft[:, :, : self.ldmax_high, ...], xtft[:, :, -self.ldmax_low :, ...]],
            dim=2,
        )
        ...
        # truncate the modes
        yot = torch.cat(
            [yo[..., : self.lhmax_high, :], yo[..., -self.lhmax_low :, :]], dim=3
        )
...
class InverseRealFFT3(nn.Module):
    ...
    def forward(self, x):
        ...
        out = torch.fft.irfftn(
            xt, s=(self.nd, self.nh, self.nw), dim=(-3, -2, -1), norm="ortho"
        )
