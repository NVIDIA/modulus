...
for year in years:
    ...
    print(year)
    src = (
        "/global/cscratch1/sd/jpathak/ERA5_u10v10t2m_netcdf/10m_u_v_2m_t_gp_lsm_toa_"
        + str(year)
        + ".nc"
    )
    ...
    src = (
        "/project/projectdirs/dasrepo/jpathak/ERA5_more_data/z_u_v_1000_"
        + str(year)
        + ".nc"
    )
    ...
    usr = dir_dict[year]
    src = (
        "/project/projectdirs/dasrepo/ERA5/wind_levels/6hr/"
        + usr
        + "/u_v_z_pressure_level_850_"
        + str(year)
        + ".nc"
    )
