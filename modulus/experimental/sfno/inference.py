...
if __name__ == "__main__":
    ...
    if (
        args.enable_odirect
        and os.environ.get(odirect_env_var_name, "False").lower() != "true"
    ):
        raise RuntimeError(
            f"Error, {odirect_env_var_name} h5py with MPI support is not installed. Please refer to README for instructions on how to install it."
        )
    ...
    if "metadata_json_path" in params:
        ...
        raise RuntimeError(
            f"Error, please specify a dataset descriptor file in json format"
        )
