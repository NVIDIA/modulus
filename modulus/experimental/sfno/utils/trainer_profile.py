...
class Trainer:
    ...
    def __init__(self, params, world_rank):
        ...
        if hasattr(params, "amp_mode") and (params.amp_mode != "none"):
            ...
            if params.log_to_screen:
                self.logger.info(
                    "Enabling automatic mixed precision in {params.amp_mode}."
                )
        else:
    ...
    def train_one_epoch(self):
        ...
        with torch.autograd.profiler.emit_nvtx(
            enabled=True, record_shapes=self.enable_anna
        ):
            ...
            for data in tqdm(
                ...
                if (self.capture_mode == "training") and (
                    self.iters == self.capture_range_start
                ):
                ...
                # stop profiling
                if (self.capture_mode == "training") and (
                    self.iters >= self.capture_range_stop
                ):
            ...
            with torch.inference_mode():
                with torch.no_grad():
                    ...
                    for data in tqdm(
                        ...
                        # profile validation
                        if (self.capture_mode == "validation") and (
                            eval_steps == self.capture_range_start
                        ):
                            cudaProfilerStart(self.device, self.profile_this_rank)
                        ...
                        # exit eval
                        if (self.capture_mode == "validation") and (
                            eval_steps >= self.capture_range_stop
                        ):
                            cudaProfilerStop(self.device, self.profile_this_rank)
