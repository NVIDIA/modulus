...
from modulus.experimental.sfno.utils.metrics.functions import (
    GeometricACC,
    GeometricL1,
    GeometricRMSE,
    Quadrature,
...
class MetricsHandler:
    ...
    def __init__(
        ...
        self.l1_handle = torch.compile(
            self.l1_handle, mode="max-autotune-no-cudagraphs"
        )
        ...
        self.rmse_handle = torch.compile(
            self.rmse_handle, mode="max-autotune-no-cudagraphs"
        )
        ...
        self.acc_handle = torch.compile(
            self.acc_handle, mode="max-autotune-no-cudagraphs"
        )
    ...
    def initialize_buffers(self):
        ...
        self.valid_buffer_cpu = torch.zeros(
            (3), dtype=torch.float32, device="cpu", pin_memory=pin_memory
        )
        ...
        self.rmse_curve_cpu = torch.zeros(
            (self.out_channels_local, self.valid_autoreg_steps + 1),
            dtype=torch.float32,
            device="cpu",
            pin_memory=pin_memory,
    ...
    def update(self, prediction, target, loss, idt):
        ...
        # update parameters
        self.acc_curve[:, idt] += self.acc_handle(
            prediction - self.clim, target - self.clim
        )
        self.rmse_curve[:, idt] += self.mult * torch.sum(
            self.rmse_handle(prediction, target), dim=0
        )
    ...
    def finalize(self, final_inference=False):
        ...
        with torch.no_grad():
            ...
            if dist.is_initialized():
                ...
                dist.all_reduce(
                    self.rmse_curve, op=dist.ReduceOp.SUM, group=comm.get_group("data")
                )
                dist.all_reduce(
            ...
            if self.split_data_channels:
                ...
                dist.all_gather(
                    rmse_curve_list, self.rmse_curve, group=comm.get_group("matmul")
                )
            ...
            logs["metrics"]["rollouts"] = wandb.Table(
                data=table_data,
                columns=["metric type", "variable name", "time [h]", "value"],
            )
