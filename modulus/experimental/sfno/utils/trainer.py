...
from modulus.experimental.sfno.mpu.helpers import gather_uneven, sync_params
...
class Trainer:
    ...
    def __init__(self, params, world_rank, job_type="train"):
        ...
        if params.lr_warmup_steps > 0:
            if params.scheduler == "ReduceLROnPlateau":
                raise NotImplementedError(
                    "Error, warmup scheduler not implemented for ReduceLROnPlateau scheduler"
                )
        ...
        self.visualizer = visualize.VisualizationWrapper(
            ...
            lon=self.valid_dataset.grid_converter.get_dst_coords()[1].cpu().numpy()
            - np.pi,
            scale=out_scale[0, ...],
            bias=out_bias[0, ...],
    ...
    def train(self):
        ...
        for epoch in range(self.startEpoch, self.params.max_epochs):
            ...
            if (self.data_parallel_rank == 0) and (
                self.params.save_checkpoint != "none"
            ):
    ...
    def train_one_epoch(self):
        ...
        for data in tqdm(
            ...
            if (self.params.log_weights_and_grads > 0) and (
                (self.iters - 1) % self.params.log_weights_and_grads == 0
            ):
                self.log_weights_and_grads(
                    self.params["experiment_dir"], iters=self.iters, epoch=self.epoch
                )
    ...
    def restore_checkpoint(
        ...
        if checkpoint_mode == "legacy":
            ...
            if self.params.log_to_screen:
                self.logger.info(
                    "Loading checkpoint %s in legacy mode" % checkpoint_fname
                )
        ...
        # new flexible mode allows to load models in arbitrary model-parallel configurations
            ...
            if self.params.log_to_screen:
                self.logger.info(
                    "Loading checkpoint %s in flexible mode" % checkpoint_fname
                )
            checkpoint = torch.load(checkpoint_fname, map_location="cpu")
            ...
            if load_optimizer:
                ...
                # self.optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
                raise NotImplementedError(
                    "Error, restoring optimizer not supported for flexible checkpoint format yet"
                )
                ...
                for data in tqdm(
                    ...
                    for idt, targ in enumerate(tarlist):
                        ...
                        if self.preprocessor.n_history > 0:
                            prede = self.preprocessor.expand_history(
                                pred, (self.preprocessor.n_history + 1)
                            )
                            pred = prede[:, -1, ...]
                    ...
                    for idp, predt in enumerate(predictions):
                        ...
                        predte = self.preprocessor.expand_history(
                            predt, (self.preprocessor.n_history + 1)
                        )
                        if self.preprocessor.n_history > 0:
                        ...
                        for pc, tc in zip(pred_comp, targ_comp):
                            ...
                            is_same = torch.allclose(
                                pc, tc, rtol=1e-05, atol=1e-08, equal_nan=False
                            )
                            if not is_same:
    ...
    def log_weights_and_grads(
        self, weights_and_grads_path, model=None, iters=1, epoch=1
    ):
        """
        ...
        weights_and_grads_fname = os.path.join(
            ...
            f"weights_and_grads_epoch{epoch}_step{iters}_mp{mp_rank}.tar",
        )
