#!/bin/bash
#SBATCH -A devtech
#SBATCH --job-name devtech-e2prep:geometric-fcn
#SBATCH -t 03:55:00
#SBATCH -p luna
#SBATCH -N 8
#SBATCH -o geometric-fcn-%j.out
...
readonly _output_root="${BBONEV_OUTPUT}"
...
# run command
RUN_CMD="python -u train${suffix}.py --amp_mode=${amp_mode} --jit_mode=${jit_mode} ${checkpointing_args} --cuda_graph_mode=${graph_mode} ${multistep_args} --run_num=${RUN_NUM} --yaml_config=${_config_file} --config=${_config_name} --fin_parallel_size=${fin_parallel_size} --fout_parallel_size=${fout_parallel_size} --h_parallel_size=${h_parallel_size}"
