...
class Era5Dataset(torch.utils.data.Dataset):
    ...
    def _get_files_stats(self):
        ...
        self.dist.print0(
            "Number of samples per year: {}".format(self.n_samples_per_year)
        )
        self.dist.print0(
...
def get_zarr_dataset(params, train, all_times=False):
    ...
    zdataset = _ZarrDataset(
        params.train_data_path, get_target_normalization=get_target_normalization
    )
    return ZarrDataset(
        dataset=zdataset, params=params, train=train, all_times=all_times
    )
...
def get_netcdf_dataset(params):
    ...
    _zarr_dataset = _ZarrDataset(
        _zarr_path, get_target_normalization=get_target_normalization
    )
