        ...
        for path in self.files_paths:
            with h5py.File(path, "r") as _f:
                ...
                self.img_shape_x = _f["fields"].shape[
                    2
                ]  # -1   #just get rid of one of the pixels, but not for CWB
                self.img_shape_y = _f["fields"].shape[3]

        skip_steps = 1
        self.samples = [np.arange(0, len_, skip_steps) for len_ in self.len_list]
...
class CWBERA5DatasetV1(torch.utils.data.Dataset):
    ...
    def __init__(
            ...
            if k in self.filelist:
                ...
                for ii, hour_idx in enumerate(v):
                    self.data_list.append(
                        [i, ii, hour_idx]
                    )  # [file_idx, cwb_data_idx, era5_data_idx]
        ...
        self.cwb_sd = np.load(f"{self.cwb_stats_dir}/global_stds.npy")[
            :, self.chans, ...
        ]  # has shape [1, C, 1, 1]
    ...
    def __getitem__(self, idx):
        ...
        x_era5 = self.era5_data_files[file_idx]["fields"][
            era5_data_idx, :, 721 - 472 : 721 - 438, 465:503
        ]
        x_era5 = x_era5[:, ::-1, :]
        x_era5 = np.stack(
            [x_era5[chan, ...] for chan in self.align_era5_chans],
            axis=0,
        )
...
class CWBERA5DatasetV2(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        for i, (k, v) in enumerate(all_ranges.items()):
            if self.filelist:
                if k in self.filelist:
                    self.cwb_data_paths.append(os.path.join(self.cwb_data_dir, k))
                    self.era5_data_paths.append(
                        os.path.join(os.path.join(self.era5_data_dir, k[0:4]), k)
                    )
                    for ii, hour_idx in enumerate(v):
                        self.data_list.append(
                            [i, ii, hour_idx]
    ...
    def __getitem__(self, idx):
        ...
        x_era5 = self._create_highres_(
            x_era5, shape=(x_cwb.shape[1], x_cwb.shape[2])
        )  # 20x448x448
...
class ZarrDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, params, path: str, train=True):
        self.path = path
        ...
        self.in_channels = params.in_channels
        self.out_channels = params.out_channels
        self.normalize = params.normalization
    ...
    def __getitem__(self, idx):
        ...
        target = normalize(target, self.group["cwb_center"], self.group["cwb_scale"])
    ...
    def input_channels(self):
        ...
        variable = self.group["era5_variable"][self.in_channels]
        level = self.group["era5_pressure"][self.in_channels]
    ...
    def output_channels(self):
        ...
        variable = self.group["cwb_variable"][self.out_channels]
        level = self.group["cwb_pressure"][self.out_channels]
        return [{"variable": v, "pressure": lev} for v, lev in zip(variable, level)]
