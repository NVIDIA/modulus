...
class MultifilesDataset(Dataset):
    ...
    def __init__(
        ...
        if params.normalization == "minmax":
            ...
            self.out_scale = (
                np.load(params.max_path)[:, self.out_channels] - self.out_bias
            )
    ...
    def _get_files_stats(self, enable_logging):
        ...
        if not self.files_paths:
            raise IOError(
                f"Error, the specified file path {self.location} does not contain h5 files."
            )
        ...
        # compute padding
        read_pad_x = (
            (self.crop_size[0] + self.io_grid[0] - 1) // self.io_grid[0]
        ) - read_shape_x
        read_pad_y = (
            (self.crop_size[1] + self.io_grid[1] - 1) // self.io_grid[1]
        ) - read_shape_y
    ...
    def __getitem__(self, global_idx):
        ...
        for offset_idx in range(self.n_history + 1):
            year_idx = (
                bisect_right(self.year_offsets, global_idx + self.dt * offset_idx) - 1
            )
        ...
        if self.add_zenith:
            ...
            if (self.read_pad[0] > 0) or (self.read_pad[1] > 0):
                zen_inp = np.pad(
                    zen_inp,
                    [(0, 0), (0, 0), (0, self.read_pad[0]), (0, self.read_pad[1])],
                )
                zen_tar = np.pad(
                    zen_tar,
                    [(0, 0), (0, 0), (0, self.read_pad[0]), (0, self.read_pad[1])],
