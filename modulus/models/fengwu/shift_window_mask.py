...
def window_partition_2d(x: torch.Tensor, window_size):
    """
    Args:
        x: (B, Lat, Lon, C)
        window_size (tuple[int]): [win_lat, win_lon]

    Returns:
        windows: (B*num_lon, num_lat, win_lat, win_lon, C)
    """
    B, Lat, Lon, C = x.shape
...
def window_reverse_2d(windows, window_size, Lat, Lon):
    """
    Args:
        windows: (B*num_lon, num_lat, win_lat, win_lon, C)
        window_size (tuple[int]): [win_lat, win_lon]
...
def get_shift_window_mask_2d(input_resolution, window_size, shift_size):
    """
    Along the longitude dimension, the leftmost and rightmost indices are actually close to each other.
    If half windows apper at both leftmost and rightmost positions, they are dircetly merged into one window.
    Args:
    ...
    Returns:
        attn_mask: (n_lon, n_lat, win_lat*win_lon, win_lat*win_lon)
    """
    ...
    mask_windows = window_partition_2d(
        img_mask, window_size
    )  # n_lon, n_lat, win_lat, win_lon, 1
    mask_windows = mask_windows.view(
        mask_windows.shape[0], mask_windows.shape[1], win_lat * win_lon
    )
    attn_mask = mask_windows.unsqueeze(2) - mask_windows.unsqueeze(3)
    attn_mask = attn_mask.masked_fill(attn_mask != 0, float(-100.0)).masked_fill(
        attn_mask == 0, float(0.0)
