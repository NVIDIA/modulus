...
from modulus.utils.sfno.distributed.layers import DistributedRealFFT2, DistributedInverseRealFFT2, DistributedMLP

# more distributed stuff
from modulus.utils.sfno.distributed import comm
from modulus.utils.sfno.distributed.mappings import scatter_to_parallel_region, gather_from_parallel_region
...
class SpectralFilterLayer(nn.Module):
    def __init__(
        ...
        if filter_type == 'non-linear' and (isinstance(forward_transform, th.RealSHT) or isinstance(forward_transform, thd.DistributedRealSHT)):
        ...
        elif filter_type == 'non-linear' and (isinstance(forward_transform, RealFFT2) or isinstance(forward_transform, DistributedRealFFT2)):
        ...
        elif filter_type == 'linear' and (isinstance(forward_transform, th.RealSHT) or isinstance(forward_transform, thd.DistributedRealSHT)):
...
class SphericalFourierNeuralOperatorNet(Module):
    def __init__(
        ...
        self.img_shape = (params.img_shape_x, params.img_shape_y) if hasattr(params, "img_shape_x") and hasattr(params, "img_shape_y") else img_shape
        ...
        self.activation_function = params.activation_function if hasattr(params, "activation_function") else activation_function
        ...
        self.padding = (img_dist_h * comm.get_size("h") - self.img_shape[0], img_dist_w * comm.get_size("w") - self.img_shape[1])
        ...
        if self.spectral_transform == 'sht':
            ...
            # set up
            self.trans_down = sht_handle(*self.img_shape, lmax=modes_lat, mmax=modes_lon, grid='equiangular').float()
            self.itrans_up  = isht_handle(*self.img_shape, lmax=modes_lat, mmax=modes_lon, grid='equiangular').float()
            ...
            self.img_shape_eff = [ self.img_shape[0] + self.padding[0], self.img_shape[1] + self.padding[1] ]
            self.img_shape_loc = [ self.img_shape_eff[0] // comm.get_size("h"), self.img_shape_eff[1] // comm.get_size("w")]
        ...
        if (comm.get_size("h") > 1) or (comm.get_size("w") > 1):
            self.img_shape_loc = (self.trans_down.nlat_local, self.trans_down.nlon_local)
            self.img_shape_eff = [self.trans_down.nlat_local+self.trans_down.nlatpad_local, self.trans_down.nlon_local+self.trans_down.nlonpad_local]
        ...
        if self.normalization_layer == "layer_norm":
            norm_layer0 = partial(nn.LayerNorm, normalized_shape=(self.img_shape_loc[0], self.img_shape_loc[1]), eps=1e-6)
            norm_layer1 = partial(nn.LayerNorm, normalized_shape=(self.h_loc, self.w_loc), eps=1e-6)
        elif self.normalization_layer == "instance_norm":
            if comm.get_size("spatial") > 1:
                norm_layer0 = partial(DistributedInstanceNorm2d, num_features=self.embed_dim, eps=1e-6, affine=True)
            else:
    ...
    def forward(self, x):
        ...
        if hasattr(self, 'pos_embed'):
            ...
            if self.img_shape_loc != self.img_shape_eff:
                ...
                xp[..., :self.img_shape_loc[0], :self.img_shape_loc[1]] = x[..., :self.img_shape_loc[0], :self.img_shape_loc[1]] + self.pos_embed
                x = xp
            else:
                x = x + self.pos_embed
