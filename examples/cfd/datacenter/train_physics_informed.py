...
def dilate_mask_3d(mask, padding_size):
    ...
    kernel = torch.ones((kernel_size, kernel_size, kernel_size), dtype=torch.float32).to(mask.device)
    kernel = kernel.unsqueeze(0).unsqueeze(0)

    dilated_result = torch.clamp(torch.nn.functional.conv3d(inverted_mask, kernel, padding=padding_size), 0, 1)
...
@torch.no_grad()
def validation_step(model, dataset, pos_embed_tensor, epoch, plotting=False, device=None, name="default"):
    loss_epoch = 0.0
    num_samples = 0.0
    ...
    for i, data in enumerate(dataset):
        ...
        outvar = torch.permute(var[..., 0:5], (0, 4, 1, 2, 3))  # Grab U components, T and P
        ...
        if plotting:
            if i == 0:
                for chan in range(outvar.size(1)):
                    ...
                    vmin, vmax = np.min(outvar[i, chan, :, :, nz//2].detach().cpu().numpy()), np.max(outvar[i, chan, :, :, nz//2].detach().cpu().numpy())
                    # plot z slices
                    im = ax[0].imshow(outvar[i, chan, :, :, nz//2].detach().cpu().numpy(), vmin=vmin, vmax=vmax)
                    fig.colorbar(im, ax=ax[0])
                    im = ax[1].imshow(pred_outvar[i, chan, :, :, nz//2].detach().cpu().numpy(), vmin=vmin, vmax=vmax)
                    fig.colorbar(im, ax=ax[1])
                    im = ax[2].imshow((pred_outvar[i, chan, :, :, nz//2] - outvar[i, chan, :, :, nz//2]).detach().cpu().numpy())
...
def main(cfg: DictConfig) -> None:
    ...
    pos_embed = np.stack((xv, x_freq_sin, x_freq_cos, yv, y_freq_sin, y_freq_cos, zv, z_freq_sin, z_freq_cos), axis=0)
    ...
    pos_embed_tensor = pos_embed_tensor.repeat(cfg.train_batch_size, 1, 1, 1, 1)   # repeat along the batch size dim
    ...
    mean_dict = {'T': 39, 'U': 1.5983600616455078, 'p': 6.1226935386657715, 'wallDistance': 0.6676982045173645}
    std_dict = {'T': 4, 'U': 1.3656059503555298, 'p': 4.166020393371582, 'wallDistance': 0.45233625173568726}
    ...
    phy_informer = PhysicsInformer(
        ...
        device=dist.device,
        fd_dx=[(bounds[1]-bounds[0])/nx, (bounds[3] - bounds[2])/ny, (bounds[5] - bounds[4])/nz],
    )
    ...
    if dist.rank == 0:
        pos_embed_tensor_val = torch.from_numpy(pos_embed).to(torch.float).to(dist.device)
        pos_embed_tensor_val = pos_embed_tensor_val.repeat(cfg.val_batch_size, 1, 1, 1, 1)   # repeat along the batch size dim
    ...
    for epoch in range(max(1, loaded_epoch + 1), cfg.max_epochs + 1):  # epochs
        with LaunchLogger(
            ...
            for i, data in enumerate(dataset):
                ...
                invar = torch.permute(var[..., 5:6], (0, 4, 1, 2, 3))   # Grab Wall Distance
                invar = torch.cat((invar, pos_embed_tensor), axis=1)    # Concat along channel dim
                outvar = torch.permute(var[..., 0:5], (0, 4, 1, 2, 3))  # Grab U components, T and P
                pred_outvar = model(invar)
        ...
        if dist.rank == 0:
            with LaunchLogger("valid", epoch=epoch) as log:
                train_loss = validation_step(model, train_dataset_plotting, pos_embed_tensor_val, epoch, plotting=True, name="train")
                val_loss = validation_step(model, val_dataset, pos_embed_tensor_val, epoch, plotting=True, name="val")
