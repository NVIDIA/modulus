...
def generate_mask(points, sample):
    ...
    geo_block_neg_y = geo_block_neg_y.repeat(
        ...
        repeat_lower=(0, 0, 0),
        repeat_higher=(int(num_racks), 0, 0),
        center=(origin[0], origin[1] - width - 3 * rack_y / 2 - 1, origin[2] + rack_z / 2),
    )
        ...
        for j in range(num_cells_y - 1):
            for i in range(num_cells_x - 1):
                pt_ids = [
                    ...
                    (i+1) + (j+1)*num_cells_x + (k+1)*num_cells_x*num_cells_y,
...
def main(cfg: DictConfig) -> None:
    ...
    for design in combinations:
        ...
        gap = (length / rack) - 600
        sample = 0, rack, width, gap, 0, length, height # case num and translate var dont matter
        
        sdf, hot_aisle_bounds = generate_mask(points, sample)
        mask = np.where((sdf > 0) & (zv < hot_aisle_bounds[1][2]) & (yv > hot_aisle_bounds[0][1]) & (xv < hot_aisle_bounds[1][0]), 1, 0)
    
        sdf = ((sdf - mean_dict['wallDistance']) / std_dict['wallDistance']) * mask
    
        invar_np = np.concatenate((np.expand_dims(sdf, 0), pos_embed), axis=0)  # concat along channel dim
