...
class BiStrideMeshGraph(Module):
    """
     Parameters
    ----------
    input_dim_nodes : int
    ...
    def _EMD(self,m_ids, multi_gs, pos,graph):
        # encode first layer with edge features
        edge_features = self.edge_encoder(graph.edata["x"])
        node_features = self.node_encoder(graph.ndata["x"])
        x = self.first_layer_processor(node_features, edge_features, graph) #[num_nodes,d]
        for _ in range(self.u_net_times):
            x = self.BS_process(x, m_ids, multi_gs, pos) # This is a BSGMP class
