...
def main(cfg: DictConfig) -> None:
    ...
    for _, sample in enumerate(dataloader):
        filename = sample["filename"][0] # batch size 1, we can just pull out the filename
        ...
        try:
            ...
            if cfg.model.model_type == "volume" or cfg.model.model_type == "combined":
                print(f"{filename}: volume min/max: {torch.amin(processed_data['volume_fields'], 0)}, {torch.amax(processed_data['volume_fields'], 0)}")
            if cfg.model.model_type == "surface" or cfg.model.model_type == "combined":
                print(f"{filename}: surface min/max: {torch.amin(processed_data['surface_fields'], 0)}, {torch.amax(processed_data['surface_fields'], 0)}")
            np.save(output_file, processed_data)
            print(f"Rank {dist.rank}: Completed {filename} in {time.time() - start_time:.2f}s")
