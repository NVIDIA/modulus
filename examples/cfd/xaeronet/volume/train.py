...
from utils import find_h5_files, save_checkpoint, load_checkpoint, count_trainable_params, calculate_continuity_loss
...
def main(cfg: DictConfig) -> None:
    ...
    # Create DataLoader 
    sampler = DistributedSampler(train_dataset, num_replicas=dist.world_size, rank=dist.rank, shuffle=True, drop_last=True)
    train_dataloader = create_dataloader(train_dataset, mean, std, batch_size=1, num_workers=1, sampler=sampler)
    valid_dataloader = create_dataloader(valid_dataset, mean, std, batch_size=1, num_workers=1, sampler=None)
    ...
    data, filter, phys_filter, _ = parallel_partitioning(train_dataloader, num_partitions=cfg.num_partitions, partition_width=cfg.partition_width, halo_width=cfg.halo_width)
    vdata, vfilter, _, vbatch = parallel_partitioning(valid_dataloader, num_partitions=cfg.num_partitions, partition_width=cfg.partition_width, halo_width=cfg.halo_width)
    ...
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=cfg.num_epochs, eta_min=cfg.end_lr)
    scaler = GradScaler()
    ...
    for epoch in range(start_epoch, cfg.num_epochs):
        ...
        for i in range(len(data)):
            ...
            for idx, part in enumerate(data[i]): # (x, y, z, sdf, dsdf_dx, dsdf_dy, dsdf_dz, u_x, u_y, u_z, p)
                with torch.autocast(amp_device, enabled=True, dtype=amp_dtype):
                    part = part.to(device)
                    inp = torch.cat([part[:, 0:7],
                    ...
                    loss_data = torch.mean((pred_masked - data_masked) ** 2) / len(data[0])
                    total_loss_data += loss_data.item()
                    pred_phys_filtered = pred[:, :, list(phys_filter[i][idx])]
                    pred_phys_filtered_denormalized = pred_phys_filtered[:, 0:3] * std_tensor[None, 7:10, None, None, None] + mean_tensor[None, 7:10, None, None, None]
                    sdf_phys_filtered = part[:, 3, list(phys_filter[i][idx])]
                    sdf_phys_filtered_denormalized = sdf_phys_filtered * std[3] + mean[3]
                    loss_continuity = calculate_continuity_loss(pred_phys_filtered_denormalized, sdf_phys_filtered_denormalized) / len(data[0])
        ...
        if dist.rank==0:
            ...
            print(f"Epoch {epoch+1}, Learning Rate: {current_lr}, Data Loss: {total_loss_data/len(data)}, Continuity Loss: {cfg.continuity_lambda * total_loss_continuity/len(data)}, Total Loss: {(total_loss_data + cfg.continuity_lambda * total_loss_continuity) / len(data)}")
            writer.add_scalar("training_data_loss", total_loss_data/len(data), epoch)
            writer.add_scalar("training_continuity_loss", cfg.continuity_lambda * total_loss_continuity / len(data), epoch)
            writer.add_scalar("total_loss", (total_loss_data + cfg.continuity_lambda * total_loss_continuity) / len(data), epoch)
        ...
        if dist.rank==0 and (epoch) % cfg.validation_freq == 0:
            with torch.no_grad():
                with torch.autocast(amp_device, enabled=True, dtype=amp_dtype):
                    ...
                    for i in range(len(vdata)):
                        ...
                        err = torch.mean((pred_masked - data_masked) ** 2) / len(vdata[0])
                        valid_loss += err
            ...
            dims = np.array([x_dim, y_dim, z_dim])  # The number of voxels in each direction
