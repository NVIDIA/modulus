...
def partition_batch(batch, num_partitions, partition_width, halo_width):
    ...
    phys_filter = np.zeros((num_partitions, partition_width + 2 * halo_width), dtype=bool)
    ...
    data[num_partitions - 1] = batch[:, :, (num_partitions-1)*partition_width-2*halo_width:, :, :]
    ...
    phys_filter[1:num_partitions-1, halo_width-1:partition_width + halo_width + 1] = True
...
def process_batch(batch, num_partitions, partition_width, halo_width):
    data_i, filter_i, phys_filter_i = partition_batch(batch, num_partitions, partition_width, halo_width)
    return data_i, filter_i, phys_filter_i, batch

# Efficient processing of valid_dataloader batches using parallelism
def parallel_partitioning(dataloader, num_partitions=7, partition_width=100, halo_width=40):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        for batch_i in dataloader:
            futures.append(executor.submit(process_batch, batch_i, num_partitions, partition_width, halo_width))
