...
def generate_mesh_train():
    cad_mesh = trimesh.load('/home/juheonlee/juheon_work/new_data/CustomerLarge-2_003-iso_offset.ply')

    x = torch.FloatTensor(np.asarray(cad_mesh.vertices)) - torch.FloatTensor(cad_mesh.bounds.mean(0))
    
    # raw scan
    scan_pts = o3d.io.read_point_cloud('/home/juheonlee/juheon_work/new_data/CustomerLarge-2_003-iso_offset_aligned.pcd')
    y = torch.FloatTensor(np.asarray(scan_pts.points))  - torch.FloatTensor(cad_mesh.bounds.mean(0))
...
def generate_mesh_eval(view=False):
    object_name = 'bar'
    part_id = 5
    cad_mesh = trimesh.load('%s_%d/cad/%s_%d_uptess.obj'%(object_name,part_id,object_name,part_id))
    scan_pts = np.loadtxt('C:/Users/leejuhe/Documents/comp/out__%02d.csv'%(part_id),delimiter=',')
...
def generate_scan_file(root_dir="/home/chenle/codes/DL_prediction_compensation-master/data",
                       save_csv=True,
                       part_id=0,
                       part_name=None
                       ):
    ...
    scan_pts = o3d.io.read_point_cloud(glob.glob(f"{root_dir}/{part_id}/scan/*.{'pcd'}")[0])
...
def generate_scan():
    ...
    for i in range(len(subfolders)):
        ...
        if part_id == '23': continue
        cad_mesh = trimesh.load('%s/%s_%s/cad/%s_%s_uptess.obj'%(path1,object_name,part_id,object_name,part_id))
        print(cad_mesh)
        x = torch.FloatTensor(np.asarray(cad_mesh.vertices)) - torch.FloatTensor(cad_mesh.bounds.mean(0))

        # raw scan
        scan_pts = o3d.io.read_point_cloud('%s/scan/%s_scan_u_aligned.pcd'%(subfolders[i],part_id))
        y = torch.FloatTensor(np.asarray(scan_pts.points))  - torch.FloatTensor(cad_mesh.bounds.mean(0))
        ...
        vertices = new_vert + torch.FloatTensor(cad_mesh.bounds.mean(0))
        print(vertices)
        torch.save(vertices,'%s/scan/%s_%s_test.pth'%(subfolders[i],object_name,part_id))
    #'''
