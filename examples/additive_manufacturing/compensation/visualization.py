...
def plot_mesh(mesh, ax, face_color, edge_color, label):
    ...
    ax.add_collection3d(Poly3DCollection(vertices[triangles], facecolor=face_color, edgecolor=edge_color, alpha=0.3))
    ax.scatter(vertices[:, 0], vertices[:, 1], vertices[:, 2], color=edge_color, s=0.1)  # Points
...
uncompensated_verts_tensor = torch.tensor(uncompensated_verts, dtype=torch.float32).unsqueeze(0)
compensated_verts_tensor = torch.tensor(compensated_verts, dtype=torch.float32).unsqueeze(0)
...
uncomp_rms = calculate_rms(
    uncompensated_verts_tensor[:, :min_len, :],
    target_verts_tensor[:, :min_len, :]
)
comp_rms = calculate_rms(
    compensated_verts_tensor[:, :min_len, :],
    target_verts_tensor[:, :min_len, :]
)
