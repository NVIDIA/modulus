...
def halo_padding_1d(
    ...
    dim : int, optional
        Dimension for the halo reduction.  Mandatory if the shape of the mesh is not 1D
    skip_dims : int, default=2
        How many dimensions of the tensor to skip in the halo selection.  Assuming most
        tensors have shape [B, C, ....] where ... represents spatial dimensions, the default is 2
    Returns
    -------
    torch.Tensor
    ...
    if dim is None:
        assert len(mesh.shape) == 1, f"Halo padding requires `dim` to be set for mesh size greater than 1 (got shape {mesh.shape})"
        dim = 0
    
    # Check the dimension fits the mesh:
    if len(mesh.shape) != 0:
        assert dim < len(mesh.shape), f"Halo padding can not pad dimension {dim} for mesh of shape {mesh.shape} (size {len(mesh.shape)})."
...
class HaloPaddingND(torch.autograd.Function):
    ...
    def forward(
        ...
        assert len(halo) == len(mesh.shape)

        padded_tensor = halo_padding_1d(dtensor.to_local(), mesh, halo[0], edge_padding_t, edge_padding_s[0])
        ctx.mesh = mesh
        ctx.halo = halo
