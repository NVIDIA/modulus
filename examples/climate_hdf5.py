...
class ClimateHDF5Datapipe(Datapipe):
    ...
    def _load_land_sea_mask(self) -> None:
        ...
        lsm = np.flip(lsm, axis=1) # flip latitude axis, TODO hacky fix and we should get this from the file
    ...
    def _load_geopotential(self, normalize: bool = True) -> None:
        ...
        geop = np.flip(geop, axis=1) # flip latitude axis, TODO hacky fix and we should get this from the file
        if normalize:
    ...
    def _load_latlon(self) -> None:
        ...
        self.cos_latlon = dali.types.Constant(np.stack((cos_lat, sin_lon, cos_lon), axis=0))
