...
def main(cfg: DictConfig) -> None:
    ...
    training_duration = getattr(cfg, "training_duration", 200000)
    total_batch_size = getattr(cfg, "total_batch_size", 256)
    batch_size_per_gpu = getattr(cfg, "batch_size_per_gpu", 2)
    ...
    enable_cudnn_benchmark = getattr(cfg, "enable_cudnn_benchmark", True)
    dataloader_workers = getattr(cfg, "dataloader_workers", 4)
    songunet_checkpoint_level = getattr(cfg, "songunet_checkpoint_level", 0)

    # Parse I/O-related options
    print_progress_freq = getattr(cfg, "print_progress_freq", 1)
    save_checkpoint_freq = getattr(cfg, "save_checkpoint_freq", 500)
    validation_freq = getattr(cfg, "validation_freq", 500)
    ...
    logger0.info(f"Batch size:              {c.total_batch_size}")
