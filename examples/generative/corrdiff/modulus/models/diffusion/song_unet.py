    ...
    def forward(self, x, noise_labels, class_labels, augment_labels=None):
        with nvtx.annotate(message="SongUNet", color="blue") if self.profile_mode else contextlib.nullcontext():
            ...
            for name, block in self.enc.items():
                with nvtx.annotate(f"SongUNet encoder: {name}", color="blue") if self.profile_mode else contextlib.nullcontext():
                    if "aux_down" in name:
                        aux = block(aux)
            ...
            for name, block in self.dec.items():
                with nvtx.annotate(f"SongUNet decoder: {name}", color="blue") if self.profile_mode else contextlib.nullcontext():
                    if "aux_up" in name:
                        aux = block(aux)
    ...
    def __init__(
        ...
        for level, mult in enumerate(channel_mult):
            ...
            if level == 0:
                ...
                self.enc[f"{res}x{res}_down"] = UNetBlock_Opt(
                    in_channels=cout, out_channels=cout, down=True, **block_kwargs
                )
                if encoder_type == "skip":
                    self.enc[f"{res}x{res}_aux_down"] = Conv2d(
            ...
            for idx in range(num_blocks):
                ...
                cout = model_channels * mult
                attn = res in attn_resolutions
                self.enc[f"{res}x{res}_block{idx}"] = UNetBlock_Opt(
                    in_channels=cin, out_channels=cout, attention=attn, **block_kwargs
                )
        skips = [
            block.out_channels for name, block in self.enc.items() if "aux" not in name
        ...
        for level, mult in reversed(list(enumerate(channel_mult))):
            res = self.img_shape_y >> level
            if level == len(channel_mult) - 1:
                self.dec[f"{res}x{res}_in0"] = UNetBlock_Opt(
                    in_channels=cout, out_channels=cout, attention=True, **block_kwargs
                )
                self.dec[f"{res}x{res}_in1"] = UNetBlock_Opt(
                    in_channels=cout, out_channels=cout, **block_kwargs
                )
            else:
                self.dec[f"{res}x{res}_up"] = UNetBlock_Opt(
                    in_channels=cout, out_channels=cout, up=True, **block_kwargs
                )
            for idx in range(num_blocks + 1):
                cin = cout + skips.pop()
                cout = model_channels * mult
                attn = idx == num_blocks and res in attn_resolutions
                self.dec[f"{res}x{res}_block{idx}"] = UNetBlock_Opt(
                    in_channels=cin, out_channels=cout, attention=attn, **block_kwargs
                )
            if decoder_type == "skip" or level == 0:
                if decoder_type == "skip" and level < len(channel_mult) - 1:
                ...
                self.dec[f"{res}x{res}_aux_norm"] = GroupNorm(
                    num_channels=cout, eps=1e-6,use_apex_gn=True
                )
                self.dec[f"{res}x{res}_aux_conv"] = Conv2d(
                    in_channels=cout, out_channels=out_channels, kernel=3, **init_zero
                )

    # @nvtx.annotate(message="SongUNetOpt", color="blue")
    def forward(self, x, noise_labels, class_labels, augment_labels=None):
        if self.embedding_type != "zero":
            # Mapping.
            emb = self.map_noise(noise_labels)
        ...
        for name, block in self.enc.items():
            with nvtx.annotate(f"SongUNet encoder: {name}", color="blue"):
                if "aux_down" in name:
                    ...
                    # For UNetBlocks check if we should use gradient checkpointing
                    if isinstance(block, UNetBlock_Opt):
                        x = block(x, emb)
                    else:
                        x = block(x)
                    skips.append(x)
...
class SongUNetPosOptEmbd(SongUNetOpt):
    """
    Reimplementation of the DDPM++ and NCSN++ architectures, U-Net variants with
    optional self-attention,embeddings, and encoder-decoder components.
    ...
    def __init__(
        ...
        if self.gridtype == "learnable": 
            ...
            self.pos_embd = self._get_positional_embedding().pin_memory() #buffer
    ...
    def positional_embedding_indexing(self, x, global_index):
        #if not learnable, store as a buffer
        if self.gridtype != "learnable":
