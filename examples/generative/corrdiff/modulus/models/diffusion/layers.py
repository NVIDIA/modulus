...
class UNetBlock_Opt(torch.nn.Module):
    """
    Unified U-Net block with optional up/downsampling and self-attention. Represents
    the union of all features employed by the DDPM++, NCSN++, and ADM architectures.
    ...
    def __init__(
        ...
        self.norm0 = GroupNorm(num_channels=in_channels, eps=eps, use_apex_gn=True, act="silu")
        self.conv0 = Conv2d(
            in_channels=in_channels,
            out_channels=out_channels,
            kernel=3,
        ...
        if self.adaptive_scale:
            self.norm1 = GroupNorm(num_channels=out_channels, eps=eps, use_apex_gn=True)
        else:
            self.norm1 = GroupNorm(num_channels=out_channels, eps=eps, use_apex_gn=True, act="silu")
        self.conv1 = Conv2d(
            in_channels=out_channels, out_channels=out_channels, kernel=3,fused_conv_bias=fused_conv_bias, **init_zero
        )
        ...
        if self.num_heads:
            self.norm2 = GroupNorm(num_channels=out_channels, eps=eps, use_apex_gn=True)
            self.qkv = Conv2d(
                in_channels=out_channels,
                out_channels=out_channels * 3,
                kernel=1,
