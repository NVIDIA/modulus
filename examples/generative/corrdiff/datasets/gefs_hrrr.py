...
gefs_isobaric_channels = ['u1000', 'u925', 'u850', 'u700', 'u500', 'u250', 'v1000', 'v925', 'v850', 'v700', 'v500', 'v250', 'z1000', 'z925', 'z850', 'z700', 'z500', 'z200', 't1000', 't925', 't850', 't700', 't500', 't100',  'r1000', 'r925', 'r850', 'r700', 'r500', 'r100']
...
def get_dataset(*, train, **kwargs):
    return HrrrForecastGEFSSyntheticDataset(
        **kwargs,
        train=train
    )

class HrrrForecastGEFSSyntheticDataset(DownscalingDataset):
    '''
    ...
    def __init__(
        ...
        *,
        ...
        out_channels: Iterable[str] = ("u10m", "v10m", "t2m", "precip", "cat_snow", "cat_ice", "cat_freez", "cat_rain", "cat_none"),
        gefs_surface_channels: Iterable[str] = ("u10m", "v10m", "t2m", "q2m", "sp", "msl", "precipitable_water"),
        gefs_isobaric_channels: Iterable[str] = ('u1000', 'u925', 'u850', 'u700', 'u500', 'u250', 'v1000', 'v925', 'v850', 'v700', 'v500', 'v250', 'z1000', 'z925', 'z850', 'z700', 'z500', 'z200', 't1000', 't925', 't850', 't700', 't500', 't100',  'r1000', 'r925', 'r850', 'r700', 'r500', 'r100'),
    ...
    def time(self):
        return ["2024011212f00", "2024011312f00", "2024011412f00", "2024011512f00"] + 9996 * ["0"]

    def input_channels(self):
        return gefs_surface_channels + gefs_isobaric_channels

    def output_channels(self):
        return ["u10m", "v10m", "t2m", "precip", "cat_snow", "cat_ice", "cat_freez", "cat_rain", "cat_none"]

    def image_shape(self):
        return (1056, 1792)

    def latitude(self):
        return torch.zeros(1056, 1792)

    def longitude(self):
        return torch.zeros(1056, 1792)
    
    def __getitem__(self, global_idx):
        synthetic_hrrr = torch.randn(9, 1056, 1792)
        synthetic_gefs = torch.randn(37, 1056, 1792)
        return synthetic_hrrr, synthetic_gefs, 1, 0
...
class HrrrForecastGEFSDataset(DownscalingDataset):
    ...
    def __init__(
        ...
        *,
        ...
        self.prob_channel_index = [self.output_variables_load.index(prob_channel) for prob_channel in self.prob_channels] 
    ...
    def _gefs_name_edit(self):
        ...
        for i in range(len(self.input_isobaric_variables_load)):
            if self.input_isobaric_variables_load[i]=="z200":
                ...
                self.input_isobaric_variables_load[i] = self.input_isobaric_variables_load[i].replace("r", "q")
    ...
    def _get_files_stats(self):
        ...
        gefs_surface_paths = glob.glob(os.path.join(self.data_path, "gefs", "*surface*.zarr"), recursive=True)
        ...
        # HRRR parsing
        self.ds_hrrr = {}
        hrrr_paths = glob.glob(os.path.join(self.data_path, "hrrr_forecast", "*[!backup].zarr"), recursive=True)
    ...
    def compute_total_samples(self):
        ...
        if last_year == 2024:
            last_sample = datetime(year=2024, month=7, day=29, hour=18, minute=0, second=0)
        else:
    ...
    def _get_hrrr(self, ts, crop_box):
        ...
        hrrr_field = hrrr_handle.sel(time=ts, channel=self.output_variables_load)["values"][..., y0:y1, x0:x1].values  
        if (len(hrrr_field.shape) == 4):
            hrrr_field = hrrr_field[0]
        
        is_precip = np.sum(hrrr_field[self.prob_channel_index, ], axis=0, keepdims = True)
        ...
        hrrr_field[prob_channel_index] = hrrr_field[prob_channel_index]/np.sum(hrrr_field[prob_channel_index, ], axis=0, keepdims = True)
