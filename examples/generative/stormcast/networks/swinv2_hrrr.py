...
def swinv2net(params, checkpoint_stages=False):
    return SwinTransformerV2Cr(
                  ...
                  masked_pretrain=params.mask_ratio>0,
                  nonzero_mask=params.nonzero_mask
    )
...
class SwinTransformerV2Cr(nn.Module):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        img_size = era5_img_size # this is the image size after patch merging etc.
        window_size = tuple([
        ...
        self.patch_embed_era5 = PatchEmbed(
            ...
            embed_dim=embed_dim//2 if self.boundary_padding_pixels == 0 else embed_dim,
            norm_layer=norm_layer,
        )
        ...
        patch_grid_size: Tuple[int, int] = patch_grid_size_era5
        ...
        if masked_pretrain and nonzero_mask:
            self.mask_val = nn.Parameter(torch.normal(0., 0.02, (1,)))

        if self.boundary_padding_pixels > 0:
            self.merging_mlp = nn.Linear(embed_dim, embed_dim//2, bias=False)

        if self.full_pos_embed:
            self.pos_embed_x = nn.Parameter(torch.randn(1, embed_dim//2, patch_grid_size_hrrr[0], patch_grid_size_hrrr[1]) * .02)

            if self.boundary_padding_pixels == 0:
                self.pos_embed_y = nn.Parameter(torch.randn(1, embed_dim//2, patch_grid_size[0], patch_grid_size[1]) * .02)
            else:
                self.pos_embed_y = nn.Parameter(torch.randn(1, embed_dim, patch_grid_size[0], patch_grid_size[1]) * .02)
    ...
    def forward_features(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        if self.boundary_padding_pixels != 0:
            y = self.patch_merge_with_boundary(x, y)
        else:
            y = self.patch_merge(x, y)


        if self.checkpoint_stages and not torch.jit.is_scripting():
            y = checkpoint(self.stages, y, use_reentrant=False)
        else:
            y = self.stages(y)
        return y
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, mask: Optional[torch.BoolTensor] ) -> torch.Tensor:
        if self.masked_pretrain:
            if self.nonzero_mask:
                x = torch.where(mask > 0, self.mask_val, x)
            else:
                x = x*mask
