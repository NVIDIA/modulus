...
def main(cfg: DictConfig) -> None:
    ...
    datapipe = DataPipe(
        data_dir=to_absolute_path(os.path.join("/data/era5_75var", "train")),
        stats_dir=to_absolute_path(os.path.join("/data/era5_75var", "stats")),
        channels=[i for i in range(73)],
    ...
    for i, data in enumerate(datapipe):       
        ...
        mean += torch.mean(diff,dim=(2,3))/len(datapipe)
        mean_sqr += torch.mean(diff_sqr,dim=(2,3))/len(datapipe)

        if i %100 ==0 and i!=0 and dist.rank==0:            
            print("saved till %d" % i)
