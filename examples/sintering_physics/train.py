...
def Train(rank_zero_logger, dist):
    # config dataset
    dataset = GraphDataset(size=C.num_steps, data_path=C.data_path, batch_size=C.batch_size,
                           prefetch_buffer_size=C.prefetch_buffer_size)
    testDataset = GraphDataset(size=C.num_steps,split='test', data_path=C.data_path, batch_size=C.batch_size)
    ...
    context_stats = Stats(torch.DoubleTensor(cast(metadata['context_mean'])),
                          torch.DoubleTensor(utils._combine_std(cast(metadata['context_std']), C.noise_std)))
    ...
    for features, targets in tqdm(dataset):
        ...
        sampled_noise = model.get_random_walk_noise_for_position_sequence(inputs, noise_std_last_step=C.noise_std)
...
def Test(rank_zero_logger):
    dataset = GraphDataset(mode='rollout', split=C.eval_split, data_path=C.data_path, batch_size=C.batch_size)
