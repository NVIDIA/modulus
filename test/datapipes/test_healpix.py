...
def test_create_time_series(data_dir, dataset_name, create_path):
    ...
    base[test_var]= np.log(
            ...
            ) - np.log(scaling[test_var]["log_epsilon"])
    ds = create_time_series_dataset_classic(
...
def test_TimeSeriesDataset_initialization(data_dir, dataset_name, scaling_dict):
    ...
    with pytest.raises(
        KeyError, match=("one or more of the input data variables")
    ):
        timeseries_ds = TimeSeriesDataset(
            dataset = zarr_ds,
            data_time_step = "3h",
    ...
    with pytest.raises(
        UserWarning, match=("providing 'forecast_init_times' to TimeSeriesDataset requires `batch_size=1`")
    ):
        timeseries_ds = TimeSeriesDataset(
            dataset = zarr_ds,
            scaling=scaling_dict,
...
def test_TimeSeriesDataset_get(data_dir, dataset_name, scaling_double_dict):
    ...
    # check target data
    # need to transpose 
    targets_expected = zarr_ds.targets[batch_size].transpose("face","channel_out","height","width")
    ...
    targets_expected = zarr_ds.targets[12].transpose("face","channel_out","height","width")
    targets_expected = targets_expected.to_numpy() / 2
    ...
    inputs, targets = timeseries_ds[-1]
    targets_expected = zarr_ds.targets[-1 - batch_size].transpose("face","channel_out","height","width")
    targets_expected = targets_expected.to_numpy() / 2
    assert np.array_equal(targets[0][:,0,:,:], targets_expected)
...
def test_TimeSeriesDataModule_initialization(data_dir, create_path, dataset_name, scaling_double_dict):
    ...
    with pytest.raises(
        ValueError, match=("'data_format' must be one of")
    ):
