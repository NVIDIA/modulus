...
class UnitTester():
    ...
    def __init__(self, args):
        ...
        self.params["model_parallel_size"] = self.params["matmul_parallel_size"] * self.params["spatial_parallel_size"]
        ...
        self.params["model_parallel_sizes"] = [args.spatial_parallel_size, args.matmul_parallel_size]
        self.params["model_parallel_names"] = ["spatial", "matmul"]
        self.params["data_parallel_shared_weights"] = False 

        comm.init(self.params,
            verbose=False)
    ...
    def test_patching_stitching(self):
        ...
        y = torch.arange(img_size[0], device=self.device).unsqueeze(-1) * torch.ones(img_size[0], img_size[1], device=self.device)
        x = torch.arange(img_size[1], device=self.device).unsqueeze(0) * torch.ones(img_size[0], img_size[1], device=self.device)

        dummy_input = (x*y).unsqueeze(0).repeat(channels_size, 1, 1) \
                        + torch.arange(channels_size, device=self.device).unsqueeze(-1).unsqueeze(-1)
        dummy_input = dummy_input.unsqueeze(0).repeat(local_batch_size, 1, 1, 1) \
                        * torch.arange(start=1, end=local_batch_size+1, device=self.device).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
        ...
        level = 0
        print(f"difference at the 0-th level: {torch.norm(dummy_stitched[:, level*channels_size:(level+1)*channels_size] - dummy_input)}")

        print(f"Computing patching locally for reference: ")
        dummy_reference = multigrid_patches(dummy_input, self.params.levels, padding)
        dummy_reference = stitch_patches(dummy_reference, 2**self.params.levels, padding, mode='batch-wise')
        print(f"Comparison to locally patched version: {torch.norm(dummy_stitched - dummy_reference)}")
    ...
    def test_mg_dataloader(self, max_iters=10, train=True):
        ...
        if train == True:
            dataloader, _dataset, _ = get_dataloader(self.params, self.params.train_data_path, dist.is_initialized(), train=True)
        else:
            dataloader, dataset = get_dataloader(self.params, self.params.valid_data_path, dist.is_initialized(), train=False)

        for i, data in enumerate(dataloader, 0):
            # data_start = time.perf_counter_ns()

            inp, tar = map(lambda x: torch.squeeze(x.to(self.device, dtype = torch.float32), dim=1), data)
            ...
            for rank in range(comm.get_world_size()):
                if rank == comm.get_world_rank():
                    ...
                    print(f"RANK {comm.get_world_rank()} MODEL ID {model_id}: sum = {inp_sum}, min = {inp_min}, max = {inp_max}", flush=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--spatial_parallel_size", default=1, type=int, help="Spatial parallelism dimension, only applicable to AFNO")
    parser.add_argument("--model_parallel_stride", default=1, type=int, help="Stride used for defining model parallel communicators") 
    ...
    # multistep stuff
    parser.add_argument("--multistep_pipeline", action='store_true', help="If enabled, multistep training steps will be folded into batch dimension")
