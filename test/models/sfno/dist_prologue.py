...
def sync_weights(prl_loc, prl_dist):
    ...
    with torch.no_grad():
        ...
        prl_dist.pos_embed.copy_(
            torch.split(prl_loc.pos_embed, lsize, dim=1)[mp_rank].contiguous()
        )
        # proj
        # weight
        lsize = prl_loc.patch_embed.proj.weight.shape[0] // mp_size
        prl_dist.patch_embed.proj.weight.copy_(
            torch.split(prl_loc.patch_embed.proj.weight, lsize, dim=0)[
                mp_rank
            ].contiguous()
        )
        # bias
        lsize = prl_loc.patch_embed.proj.bias.shape[0] // mp_size
        prl_dist.patch_embed.proj.bias.copy_(
            torch.split(prl_loc.patch_embed.proj.bias, lsize, dim=0)[
                mp_rank
            ].contiguous()
        )
...
def main(args, verify):
    ...
    # models
    model_dist = DistributedPrologue(
        params,
        img_size=(H, W),
        embed_dim=C,
    ...
    model_loc = Prologue(
        params, img_size=(H, W), embed_dim=C, num_blocks=num_blocks
    ).to(device)
