...
def sync_weights(mlp_loc, mlp_dist):
    if isinstance(mlp_loc, MLPv2):
        weight_list_loc = [mlp_loc.fc1.weight, mlp_loc.fc1.bias,
                           mlp_loc.fc2.weight, mlp_loc.fc2.bias]
    else:
        weight_list_loc = [mlp_loc.w1, mlp_loc.b1,
                           mlp_loc.w2, mlp_loc.b2]

    weight_list_dist = [mlp_dist.w1, mlp_dist.b1,
                        mlp_dist.w2, mlp_dist.b2]
    ...
    with torch.no_grad():
        ...
        weight_list_dist[2].copy_(torch.split(weight_list_loc[2], lsize, dim=1)[mp_rank])
...
def get_grads(mlp_loc, inp_loc, mlp_dist, inp_dist):
    if isinstance(mlp_loc, MLPv2):
        grad_list_loc = {"w1": mlp_loc.fc1.weight.grad, "b1": mlp_loc.fc1.bias.grad,
                         "w2": mlp_loc.fc2.weight.grad, "b2": mlp_loc.fc2.bias.grad}
    else:
        grad_list_loc = {"w1": mlp_loc.w1.grad, "b1": mlp_loc.b1.grad,
                         "w2": mlp_loc.w2.grad, "b2": mlp_loc.b2.grad}
    ...
    with torch.no_grad():
        # w1
        tmp_list = [torch.empty_like(mlp_dist.w1.grad) for _ in range(comm.get_size("matmul"))]
        ...
        # b1
        tmp_list = [torch.empty_like(mlp_dist.b1.grad) for _ in range(comm.get_size("matmul"))]
        ...
        # w2
        tmp_list = [torch.empty_like(mlp_dist.w2.grad) for _ in range(comm.get_size("matmul"))]
...
def main(args, verify):
    ...
    # models
    model_dist = DistributedMLPv2(C, hidden_size_mult*C, act_layer=nn.GELU, drop=0.,
                                  input_is_matmul_parallel=True,
                                  output_is_matmul_parallel=False).to(device)
    model_loc = MLPv2(C, hidden_size_mult*C, act_layer=nn.GELU, drop=0.).to(device)
