...
def sync_weights(block_loc, block_dist):
    weight_list_loc = [
        ...
        block_loc.filter.w2,
        block_loc.filter.b2,
        block_loc.mlp.fc1.weight,
        block_loc.mlp.fc1.bias,
        block_loc.mlp.fc2.weight,
        block_loc.mlp.fc2.bias,
        block_loc.norm1.weight,
        block_loc.norm1.bias,
        block_loc.norm2.weight,
        block_loc.norm2.bias,
    ]

    weight_list_dist = [
        block_dist.filter.w1,
        block_dist.filter.b1,
        block_dist.filter.w2,
        block_dist.filter.b2,
        block_dist.mlp.w1,
        block_dist.mlp.b1,
        block_dist.mlp.w2,
        block_dist.mlp.b2,
        block_dist.norm1.weight,
        block_dist.norm1.bias,
        block_dist.norm2.weight,
        block_dist.norm2.bias,
    ]
    ...
    with torch.no_grad():
        ...
        weight_list_dist[6].copy_(
            torch.split(weight_list_loc[6], lsize, dim=1)[mp_rank]
        )
...
def get_grads(block_loc, inp_loc, block_dist, inp_dist):
    grad_list_loc = {
        "filter.w1": block_loc.filter.w1.grad,
        "filter.b1": block_loc.filter.b1.grad,
        "filter.w2": block_loc.filter.w2.grad,
        "filter.b2": block_loc.filter.b2.grad,
        "mlp.w1": block_loc.mlp.fc1.weight.grad,
        "mlp.b1": block_loc.mlp.fc1.bias.grad,
        "mlp.w2": block_loc.mlp.fc2.weight.grad,
        "mlp.b2": block_loc.mlp.fc2.bias.grad,
        "norm1.w": block_loc.norm1.weight.grad,
        "norm1.b": block_loc.norm1.bias.grad,
        "norm2.w": block_loc.norm2.weight.grad,
        "norm2.b": block_loc.norm2.bias.grad,
    }
    ...
    with torch.no_grad():
        ...
        # w1
        tmp_list = [
            torch.empty_like(block_dist.filter.w1.grad)
            for _ in range(comm.get_size("matmul"))
        ]
        tmp_list[comm.get_rank("matmul")] = block_dist.filter.w1.grad
        dist.all_gather(
            tmp_list, block_dist.filter.w1.grad, group=comm.get_group("matmul")
        )
        grad_list_dist["filter.w1"] = torch.cat(tmp_list, dim=0).contiguous()
        # b1
        tmp_list = [
            torch.empty_like(block_dist.filter.b1.grad)
            for _ in range(comm.get_size("matmul"))
        ]
        tmp_list[comm.get_rank("matmul")] = block_dist.filter.b1.grad
        dist.all_gather(
            tmp_list, block_dist.filter.b1.grad, group=comm.get_group("matmul")
        )
        grad_list_dist["filter.b1"] = torch.cat(tmp_list, dim=0).contiguous()
        # w2
        tmp_list = [
            torch.empty_like(block_dist.filter.w2.grad)
            for _ in range(comm.get_size("matmul"))
        ]
        tmp_list[comm.get_rank("matmul")] = block_dist.filter.w2.grad
        dist.all_gather(
            tmp_list, block_dist.filter.w2.grad, group=comm.get_group("matmul")
        )
        grad_list_dist["filter.w2"] = torch.cat(tmp_list, dim=0).contiguous()
        # b2
        tmp_list = [
            torch.empty_like(block_dist.filter.b2.grad)
            for _ in range(comm.get_size("matmul"))
        ]
        tmp_list[comm.get_rank("matmul")] = block_dist.filter.b2.grad
        dist.all_gather(
            tmp_list, block_dist.filter.b2.grad, group=comm.get_group("matmul")
        )
        ...
        # w1
        tmp_list = [
            torch.empty_like(block_dist.mlp.w1.grad)
            for _ in range(comm.get_size("matmul"))
        ]
        tmp_list[comm.get_rank("matmul")] = block_dist.mlp.w1.grad
        dist.all_gather(
            tmp_list, block_dist.mlp.w1.grad, group=comm.get_group("matmul")
        )
        grad_list_dist["mlp.w1"] = torch.cat(tmp_list, dim=0).contiguous()
        # b1
        tmp_list = [
            torch.empty_like(block_dist.mlp.b1.grad)
            for _ in range(comm.get_size("matmul"))
        ]
        tmp_list[comm.get_rank("matmul")] = block_dist.mlp.b1.grad
        dist.all_gather(
            tmp_list, block_dist.mlp.b1.grad, group=comm.get_group("matmul")
        )
        grad_list_dist["mlp.b1"] = torch.cat(tmp_list, dim=0).contiguous()
        # w2
        tmp_list = [
            torch.empty_like(block_dist.mlp.w2.grad)
            for _ in range(comm.get_size("matmul"))
        ]
        tmp_list[comm.get_rank("matmul")] = block_dist.mlp.w2.grad
        dist.all_gather(
            tmp_list, block_dist.mlp.w2.grad, group=comm.get_group("matmul")
        )
        ...
        # w
        tmp_list = [
            torch.empty_like(block_dist.norm1.weight.grad)
            for _ in range(comm.get_size("matmul"))
        ]
        tmp_list[comm.get_rank("matmul")] = block_dist.norm1.weight.grad
        dist.all_gather(
            tmp_list, block_dist.norm1.weight.grad, group=comm.get_group("matmul")
        )
        grad_list_dist["norm1.w"] = torch.cat(tmp_list, dim=0).contiguous()
        # b
        tmp_list = [
            torch.empty_like(block_dist.norm1.bias.grad)
            for _ in range(comm.get_size("matmul"))
        ]
        tmp_list[comm.get_rank("matmul")] = block_dist.norm1.bias.grad
        dist.all_gather(
            tmp_list, block_dist.norm1.bias.grad, group=comm.get_group("matmul")
        )
        ...
        # w
        tmp_list = [
            torch.empty_like(block_dist.norm2.weight.grad)
            for _ in range(comm.get_size("matmul"))
        ]
        tmp_list[comm.get_rank("matmul")] = block_dist.norm2.weight.grad
        dist.all_gather(
            tmp_list, block_dist.norm2.weight.grad, group=comm.get_group("matmul")
        )
        grad_list_dist["norm2.w"] = torch.cat(tmp_list, dim=0).contiguous()
        # b
        tmp_list = [
            torch.empty_like(block_dist.norm2.bias.grad)
            for _ in range(comm.get_size("matmul"))
        ]
        tmp_list[comm.get_rank("matmul")] = block_dist.norm2.bias.grad
        dist.all_gather(
            tmp_list, block_dist.norm2.bias.grad, group=comm.get_group("matmul")
        )
...
def main(args, verify):
    ...
    # blocks
    norm_layer_dist = partial(
        nn.InstanceNorm2d,
        num_features=C_local,
        eps=1e-6,
        affine=True,
        track_running_stats=False,
    )
    model_dist = DistributedBlock(
        H,
        W,
        C,
