...
def test_topodiff_forward(device):
    ...
    assert common.validate_forward_accuracy(model,(tops, cons, timesteps,))
...
def test_topodiff_constructor(device):
    ...
    model = TopoDiff(img_resolution=args["img_resolution"], 
    ...
    assert out.shape == (bsize, args["out_channels"], args["img_resolution"], args["img_resolution"])
