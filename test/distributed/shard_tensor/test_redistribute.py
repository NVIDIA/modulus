...
from modulus.distributed import ShardTensor, DistributedManager
...
def shard_tensor_factory( mesh_names, mesh_sizes, requires_grad=False):
    ...
    domain_mesh, _, placements = init_global_shape_and_placements(mesh_names, )
    ...
    raw_data = torch.randn(local_shape, device=torch.device(f"cuda:{dm.local_rank}"), requires_grad=requires_grad)
...
def run_shard_tensor_global_reduction(rank, num_gpus, mesh_names, mesh_sizes, op, verbose):
...
def run_shard_tensor_to_replicate(rank, num_gpus, mesh_names, mesh_sizes, dst_placements, verbose):
    ...
    if verbose:
        print(f"Original shape: {shard_tensor.shape} vs Redistributed shape: {redistributed.shape}")
...
@pytest.mark.parametrize("domain_H", [2, 4])
@pytest.mark.parametrize("domain_W", [1, 2])
@pytest.mark.parametrize("redistribution_case", [
...
def test_shard_tensor_to_replicate(data_parallel_size, domain_H, domain_W, redistribution_case):
