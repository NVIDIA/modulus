...
def run_shard_tensor_initialization_from_data_rank(rank, num_gpus, mesh_names, mesh_sizes, verbose):
    ...
    domain_mesh, global_shape, placements = init_global_shape_and_placements(mesh_names, )
    ...
    for i, dim in enumerate(global_shape):
        if i in checked_dims: continue        
        assert dim == local_data.shape[i] 

def run_shard_tensor_initialization_from_all_dtensor(rank, num_gpus, mesh_names, mesh_sizes, verbose):
...
def run_shard_tensor_initialization_from_local_chunks(rank, num_gpus, mesh_names, mesh_sizes, verbose):
    ...
    st = ShardTensor.from_local(raw_data, device_mesh=domain_mesh, placements = placements, infer_shape=True)
    ...
    shard_dim_sizes = [0,]*dim_size
    dist.all_gather_object(shard_dim_sizes, replacement_size, group=domain_mesh.get_group(0))
...
def test_shard_tensor_initialization_from_data_rank(data_parallel_size, domain_H, domain_W):
    """ 
...
@pytest.mark.parametrize("domain_W", [1, ])
def test_shard_tensor_initialization_from_local_chunks(data_parallel_size, domain_H, domain_W):
    """ 
    This test is meant to ensure ShardTensor can be initialized correctly 
...
def test_shard_tensor_initialization_from_all_dtensor(data_parallel_size, domain_H, domain_W):
    """ 
    This test is meant to ensure ShardTensor can be initialized correctly 
    from local data.  Checks the following:
