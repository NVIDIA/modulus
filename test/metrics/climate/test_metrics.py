...
from modulus.metrics.climate.mse import mse
import modulus.metrics.climate.histogram as hist
import modulus.metrics.climate.ensemble_metrics as em
import modulus.metrics.climate.crps as crps
import modulus.metrics.climate.mse as mse
import modulus.metrics.climate.wasserstein as w
import modulus.metrics.climate.reduction as red
...
def test_climate_acc_mse(test_data, device, rtol: float = 1e-3, atol: float = 1e-3):
    ...
    error = mse.mse(pred_tensor**2, targ_tensor**2, dim=(1, 2))
    rerror = mse.rmse(pred_tensor**2, targ_tensor**2, dim=(1, 2))
    assert torch.allclose(
        error,
...
def test_climate_reductions(test_data, device, rtol: float = 1e-3, atol: float = 1e-3):
    ...
    pred_tensor = torch.from_numpy(pred_tensor_np).expand(channels, -1, -1).to(device)
    lat = torch.from_numpy(lat).to(device)
    weights = red._compute_lat_weights(lat)
    # Check main class
    ws = red.WeightedStatistic(weights)
    ...
    zonal_mean = red.zonal_mean(pred_tensor, lat, dim=1)
    assert torch.allclose(
        our_weighted_mean,
        zonal_mean,
        rtol=rtol,
    ...
    # Check when weights are 1 dimensional
    weights = weights.flatten()
    wv = red.WeightedVariance(weights)
    ...
    zonal_var = red.zonal_var(pred_tensor, lat, dim=1)
    zonal_std = red.zonal_var(pred_tensor, lat, dim=1, std=True)
    assert torch.allclose(
        our_weighted_var,
        zonal_var,
        rtol=rtol,
    ...
    global_mean = red.global_mean(pred_tensor, lat)
    assert torch.allclose(
    ...
    # = 16/45
    global_var = red.global_var(pred_tensor, lat)
    global_std = red.global_var(pred_tensor, lat, std=True)
    assert torch.allclose(
